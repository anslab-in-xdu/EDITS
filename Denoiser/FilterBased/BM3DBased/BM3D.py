...
Step1_Blk_Size = 2  # block_Size即块的大小
...
def Gauss_noise(img, sigma = 25):
    ...
    noise = numpy.random.normal(0, sigma, img.shape)
    res = img + noise
    return res
...
def main(eps, data):
    ...
    img_to_add_noise = ori

    # eps = 0.5
    delta = 1e-6
    sensitivity = 1
    sigma = numpy.sqrt(2 * numpy.log(1.25 / delta)) * sensitivity / eps
    # sigma = sensitivity / eps
    print("sigma:", sigma)
    Threshold_Hard3D = 2.7 * 1 * 50
    print("Threshold_Hard3D:", Threshold_Hard3D)

    img = Gauss_noise(img_to_add_noise, sigma)
    # img = ori
    
    # img[img < 0] = 0
    # img = numpy.sort(img.flatten()).reshape([50, 50])

    MSE = numpy.sum((img- ori) ** 2) / numpy.size(ori)
    #print(MSE)

    # img = numpy.sort(img.flatten()).reshape(dimensionality, dimensionality)

    # print("Noisy image:", img)
    ori_noisy_img = img

    # img_max = numpy.amax(img)
    # img = img / img_max * 255  # ------------------process-----------
    ...
    # Final_img = Final_img * img_max / 255  # ------------------process-----------
    ...
    Final_img[numpy.isnan(Final_img)] = 1
